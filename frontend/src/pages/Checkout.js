...
import {
    ...
    ShoppingBag, Loader2, Check, Plus,
    Banknote, Smartphone, Tag, X,
} from 'lucide-react';
...
const NotLoggedIn = () => (
    <div className="min-h-screen bg-khajur-cream flex flex-col items-center justify-center gap-6 px-6 text-center">
        <ShoppingBag className="w-16 h-16 text-khajur-dark/30" />
        <div>
        ...
        <Link to="/" className="bg-khajur-gold text-khajur-primary px-8 py-3 rounded-sm uppercase tracking-widest text-xs font-bold transition-all">
            Go to Home
        </Link>
    </div>
);
...
const SavedAddressCard = ({ addr, index, selected, onSelect }) => (
    <div
        ...
        className={`relative p-5 border rounded-sm cursor-pointer transition-all duration-200 ${selected ? 'border-khajur-gold bg-khajur-gold/5 shadow-[0_0_12px_rgba(198,169,98,0.2)]' : 'border-khajur-border hover:border-khajur-gold/40'}`}
    >
        {selected && (
...
// ── Order Summary ─────────────────────────────────────────────────────────────
const OrderSummary = ({
    cart, cartTotal, finalTotal, loading,
    couponData, removeCoupon, discountAmount,
}) => (
    <div className="bg-white border border-khajur-border rounded-sm sticky top-28" data-testid="checkout-summary">
        ...
        {/* Totals */}
        <div className="px-7 py-5 space-y-3 border-b border-khajur-border">
            {/* ✅ Show applied coupon as read-only (auto-applied from product page) */}
            {couponData && (
                <div className="flex items-center justify-between bg-green-50 border border-green-200 px-3 py-2.5 rounded-sm mb-3">
                    <div className="flex items-center gap-2">
                        <Tag className="w-3.5 h-3.5 text-green-600" />
                        <div>
                            <p className="text-xs font-bold text-green-700">{couponData.code} applied!</p>
                            <p className="text-xs text-green-600">You save ₹{couponData.discount_amount.toFixed(2)}</p>
                        </div>
                    </div>
                    <button
                        type="button"
        ...
        <div className="px-7 py-5">
            <button
                ...
                className="w-full flex items-center justify-center gap-2 bg-khajur-gold hover:bg-khajur-gold/90 hover:shadow-[0_0_20px_rgba(198,169,98,0.4)] disabled:opacity-60 disabled:cursor-not-allowed text-khajur-primary rounded-sm px-8 py-4 uppercase tracking-widest text-xs font-bold transition-all duration-300"
            >
                {loading
...
const Checkout = () => {
    ...
    // ✅ Coupon auto-applied from localStorage (set on product detail page)
    ...
    // ── Auto-apply coupon from localStorage ──────────────────────────────────
    useEffect(() => {
        const savedCode = localStorage.getItem('khajurkart_coupon');
        if (!savedCode || !token) return;

        const autoApply = async () => {
            try {
                const res = await axios.post(
                    `${API}/apply-coupon`,
                    { code: savedCode.toUpperCase(), order_amount: cartTotal },
                    authHeaders
                );
                setCouponData(res.data);
                toast.success(`Coupon "${res.data.code}" auto-applied! 🎉`);
            } catch {
                // Coupon may be expired or invalid — silently remove
                localStorage.removeItem('khajurkart_coupon');
            }
        };
        autoApply();
    }, [token, cartTotal]);
    ...
    // ✅ Remove coupon — also clears localStorage
    const removeCoupon = () => {
        setCouponData(null);
        localStorage.removeItem('khajurkart_coupon');
    ...
    const handleCODOrder = async () => {
        ...
        try {
            ...
            localStorage.removeItem('khajurkart_coupon'); // ✅ Clear after order
    ...
    const handleRazorpayPayment = async () => {
        setLoading(true);
        try {
            const orderRes = await axios.post(`${API}/orders`, { ...buildOrderPayload(), payment_method: 'razorpay' }, authHeaders);
            const orderId  = orderRes.data.order_id;

            const rzpOrderRes = await axios.post(`${API}/razorpay/create-order`, { amount: finalTotal, currency: 'INR' }, authHeaders);
            ...
            const options = {
                ...
                handler: async (response) => {
                    try {
                        ...
                        localStorage.removeItem('khajurkart_coupon'); // ✅ Clear after payment
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!cart.items || cart.items.length === 0) { toast.error('Your cart is empty.'); return; }
    ...
    return (
        <div className="min-h-screen bg-white py-16 md:py-24" data-testid="checkout-page">
            <div className="max-w-6xl mx-auto px-6 md:px-12">
                ...
                <div className="mb-10">
                    <Breadcrumb items={[{ label: 'Home', to: '/' }, { label: 'Cart', to: '/cart' }, { label: 'Checkout', to: '#' }]} />
                </div>
                            ...
                            <div className="bg-white border border-khajur-border rounded-sm">
                                ...
                                <div className="px-8 py-8 space-y-8">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="address" required placeholder="123, MG Road…" value={formData.address} onChange={set('address')} className={inputBase} data-testid="checkout-address" />
                                    </div>
                            ...
                            <div className="bg-white border border-khajur-border rounded-sm">
                                ...
                                <div className="px-8 py-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    {PAYMENT_METHODS.map(({ value, label, description, icon: Icon, testId }) => (
                                        <label
                                            key={value}
                                            className={`flex items-start gap-4 p-5 border rounded-sm cursor-pointer transition-all duration-200 ${formData.paymentMethod === value ? 'border-khajur-gold bg-khajur-gold/5' : 'border-khajur-border hover:border-khajur-gold/40'}`}
                                        >
